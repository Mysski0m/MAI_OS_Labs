...
void ClientApp::listenLoop() {
    ...
    if (!myPipe.create() || !myPipe.openPipe(O_RDWR)) {
        std::cerr << "\n[Error] Could not create a personal channel to receive messages.\n";
        return;
    }
        ...
        if (myPipe.receive(&pkt, sizeof(Packet))) {
            switch (pkt.type) {
                case S_MSG:
                    ...
                    break;
                case S_INVITE:
                    std::cout << "\n[INVITE]: Invite from " << pkt.sender << ". Enter '/accept " << pkt.sender << "'\n" << std::flush;
                    break;
                case S_GAME_START:
                    std::cout << "\n[GAME]: GAME HAS BEEN STARTED! Opponent: " << pkt.payload << "\n[GAME]: Your ships are automatically spaced."
                ...
                case S_BOARD:
                    ...
                    break;
                case S_SHOT_RESULT:
                    std::cout << "\n[RESULT]: " << pkt.payload << " (" << pkt.x << ", " << pkt.y << ")\n>" << std::flush;
                    break;
                case S_GAME_OVER:
                    std::cout << "\n\n====================================\n";
                    std::cout << "               GAME OVER                \n";
